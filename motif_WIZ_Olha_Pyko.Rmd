...
path_to_files <- "~/PRIMUS/data/83_BIOINFORMATICS/Kadu/motif_WIZ_Olha_Pyko/"
...
find_overlaps <- function(motif_df, gff_df) {  
  ...
  overlaps_v <- unlist(overlaps_list) %>% as.character()
...
motif_overlap_data_list <- list()
tic()
# for(i in 21:24){
for(i in seq_along(chr_names)){
    print(chr_names[i])
    
    motif_name <- paste0(chr_names[i], "_WIZZ")
    
    motif_overlap_data_list[[motif_name]] <- find_overlaps(  
      motif_df = motif_data_list[[motif_name]], 
      gff_df = human_genome_df_list[[chr_names[i]]]
      )
    
    motif_overlap_data_list[[motif_name]] %>% qs::qsave(file = paste0(path_to_files, "motif_data_overlaps_list_", motif_name, ".qs" ), nthreads = 50)
    }
toc()
...
motif_data_overlaps_files <- list.files(path = path_to_files, pattern = "motif_data_overlaps_list_chr_.*.qs", full.names = TRUE)

chr <- list.files(path = path_to_files, pattern = "motif_data_overlaps_list_chr_.*.qs") %>%
  str_remove(pattern = "motif_data_overlaps_list_") %>%
  str_remove(pattern = ".qs")

motif_data_overlaps_list <-  purrr::map(seq_along(motif_data_overlaps_files), function(i) {
  print(i)
  qs::qread(file = motif_data_overlaps_files[i], nthreads = 50)
  })

motif_data_overlaps_list <- motif_data_overlaps_list %>% purrr::set_names(chr)

motif_data_overlaps_df <- motif_data_overlaps_list %>% bind_rows(.id = "chr_")

# motif_data_overlaps_list %>% qs::qsave(file = paste0(path_to_files, "human_WIZ_motif_data_list_with_overlaps.qs"), nthreads = 50)
motif_data_overlaps_list <- qs::qread(file = paste0(path_to_files, "human_WIZ_motif_data_list_with_overlaps.qs"), nthreads = 50)

# motif_data_overlaps_df %>% qs::qsave(file = paste0(path_to_files, "human_WIZ_motif_df_with_overlaps.qs"), nthreads = 50)
motif_data_overlaps_df <- qs::qread(file = paste0(path_to_files, "human_WIZ_motif_df_with_overlaps.qs"), nthreads = 50)
...
for(i in seq_along(chr_names)){
    ...
    motif_overlap_overlap_nearest_neighbor_data_list[[motif_name]] %>% 
      qs::qsave(file = paste0(path_to_files, "motif_data_overlap_nearest_neighbor_list_", motif_name, ".qs" ), nthreads = 50)
    }
toc()
...
motif_data_overlap_nearest_neighbor_files <- list.files(path = path_to_files, pattern = "motif_data_overlap_nearest_neighbor_list_chr_.*WIZZ.qs", full.names = TRUE)

chr <- list.files(path = path_to_files, pattern = "motif_data_overlap_nearest_neighbor_list_chr_.*.qs") %>% 
  str_remove(pattern = "motif_data_overlap_nearest_neighbor_list_") %>% 
  str_remove(pattern = "_WIZZ.qs")
...
motif_data_overlap_nearest_neighbor_list <- motif_data_overlap_nearest_neighbor_list %>% purrr::set_names(chr)

motif_data_overlap_nearest_neighbor_df <- motif_data_overlap_nearest_neighbor_list %>% bind_rows(.id = "chr")

motif_data_overlap_nearest_neighbor_list %>% qs::qsave(file = paste0(path_to_files, "human_WIZ_motif_data_list_with_overlap_nearest_neighbor.qs"), nthreads = 50)
# motif_data_overlap_nearest_neighbor_list <- qs::qread(file = paste0(path_to_files, "human_WIZ_motif_data_list_with_overlap_nearest_neighbor.qs"), nthreads = 50)

motif_data_overlap_nearest_neighbor_df %>% vroom::vroom_write(file = paste0(path_to_files, "human_WIZ_motif_df_with_overlap_nearest_neighbor.tsv"), delim = "\t", num_threads = 50)
# motif_data_overlap_nearest_neighbor_df <- vroom::vroom(file = paste0(path_to_files, "human_WIZ_motif_df_with_overlap_nearest_neighbor.tsv"), delim = "\t", num_threads = 50)
