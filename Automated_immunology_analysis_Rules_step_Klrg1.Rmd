...
facs_data_df_step_Klrg1 <- facs_data_df %>% dplyr::filter(first_gate != "not_live") %>% dplyr::filter(!(second_gate %in% c("Not_Panel_A_cell_types", "CD8+_cells"))) %>% dplyr::filter(Res_Eff_Naive_gate != "Neg")

Klrg1_gate_table <- facs_data_df_step_Klrg1 %>% group_by(Klrg1_gate) %>% tally()

train_df_step_Klrg1 <- facs_data_df_step_Klrg1
```
...
val_facs_data_df_step_Klrg1 <- val_facs_data_df %>% dplyr::filter(first_gate != "not_live") %>% dplyr::filter(!(second_gate %in% c("Not_Panel_A_cell_types", "CD8+_cells"))) %>% dplyr::filter(Res_Eff_Naive_gate != "Neg")

val_Klrg1_gate_table <- val_facs_data_df_step_Klrg1 %>% group_by(Klrg1_gate) %>% tally()

val_df_step_Klrg1 <- val_facs_data_df_step_Klrg1
```
...
```{r}
library(xgboost)
step_Klrg1_levels <- train_df_step_Klrg1$Klrg1_gate %>% unique()

n_classes <- length(step_Klrg1_levels)

classes_number <- data.frame(
  classes = step_Klrg1_levels,
  Number = 1:length(step_Klrg1_levels)
)

val_label_step_Klrg1 <- val_df_step_Klrg1 %>%
  dplyr::select(Klrg1_gate) %>%
  pull() %>%
  str_replace(pattern = "Klrg1_neg", replacement = "1") %>%
  str_replace(pattern = "Klrg1_pos", replacement = "2") %>%
  as.numeric() %>%
  "-"(1)

train_label_step_Klrg1 <- train_df_step_Klrg1 %>%
  dplyr::select(Klrg1_gate) %>%
  pull() %>%
  str_replace(pattern = "Klrg1_neg", replacement = "1") %>%
  str_replace(pattern = "Klrg1_pos", replacement = "2") %>%
...
train_matrix_step_Klrg1 <- xgb.DMatrix(
  data = train_df_step_Klrg1 %>%  
    ...
    as.matrix(),
  label = train_label_step_Klrg1
  )

val_matrix_step_Klrg1 <- xgb.DMatrix(
  data = val_df_step_Klrg1 %>%  
    dplyr::select(-c(Time, File, File_scattered, cell_type, first_gate, second_gate, Res_Eff_Naive_gate, Klrg1_gate)) %>%
    # dplyr::select(-c(third_gate)) %>%
    as.matrix(),
  label = val_label_step_Klrg1
  )
...
tic()
xgb_cv_model_Klrg1 <- xgb.cv(
  params = xgb_params,
  data = train_matrix_step_Klrg1,
  nrounds = n_round,
  nfold = k_fold,
  showsd = T,
  stratified = T,
...
# xgb_cv_model_Klrg1 %>% saveRDS(paste0(source_path, "home/trufenc/xgboost_model_step_Klrg1_2020_10_23.rds"))

xgb_cv_model_Klrg1$best_iteration

xgb_cv_model_Klrg1$evaluation_log %>% dplyr::filter(iter == xgb_cv_model_Klrg1$best_iteration) %>% dplyr::select(test_merror_mean) %>% pull

xgb_cv_model_Klrg1$evaluation_log %>% 
  ggplot(aes(x = iter)) +
...
xgb_Klrg1 <- xgb.train(
  params = xgb_params, 
  data = train_matrix_step_Klrg1, 
  nrounds = xgb_cv_model_Klrg1$best_iteration,
  # nrounds = 15,
  watchlist = list(val = val_matrix_step_Klrg1, train = train_matrix_step_Klrg1), 
  print_every_n = 1, 
...
# xgb_Klrg1 %>% saveRDS(paste0(source_path, "home/trufenc/xgboost_model_step_Klrg1_2020_10_12.rds"))
# xgb_Klrg1 %>% saveRDS(paste0(source_path, "home/trufenc/xgboost_model_step_Klrg1_2020_10_22.rds"))
xgb_Klrg1 %>% saveRDS(paste0(source_path, "home/trufenc/xgboost_model_step_Klrg1_2020_11_09.rds")) # tuned hyperparameters


xgb_pred_Klrg1 <- predict(xgb_Klrg1, val_matrix_step_Klrg1)

val_prediction_df <- matrix(
  data = xgb_pred_Klrg1,
  nrow = n_classes,
  ncol = length(xgb_pred_Klrg1) / n_classes
  ) %>%
  t() %>%
  data.frame()
...
val_prediction <- val_prediction_df %>%
  mutate(
    label = val_label_step_Klrg1 %>% as.numeric() + 1,
...
importance_matrix <- xgb.importance(feature_names = colnames(train_matrix_step_Klrg1), model = xgb_Klrg1)
...
test_files_to_load_list <- purrr::map(seq_along(test_files_to_load_list), function(i) test_files_to_load_list[[i]][-c(22, 23)])
test_files_to_load_names_list <- purrr::map(seq_along(test_files_to_load_names_list), function(i) test_files_to_load_names_list[[i]][-c(22, 23)])
...
test_df_step_Klrg1 <- test_facs_data_df %>% dplyr::filter(first_gate != "not_live") %>% dplyr::filter(!(second_gate %in% c("Not_Panel_A_cell_types", "CD8+_cells"))) %>% dplyr::filter(Res_Eff_Naive_gate != "Neg")

step_Klrg1_levels <- test_df_step_Klrg1$Klrg1_gate %>% unique()

n_classes <- length(step_Klrg1_levels)

test_label <- test_df_step_Klrg1 %>%
  dplyr::select(Klrg1_gate) %>%
  pull() %>%
  str_replace(pattern = "Klrg1_neg", replacement = "1") %>%
  str_replace(pattern = "Klrg1_pos", replacement = "2") %>%
...
test_matrix <- xgb.DMatrix(
  # data = EmbedSOM_results_step_Klrg1$weights_df[-test_indices, ] %>%
  # data = test_EmbedSOM_results_step_Klrg1$weights_df %>%  
  data = test_df_step_Klrg1 %>%    
    dplyr::select(-c(Time, File, File_scattered, cell_type, first_gate, second_gate, Res_Eff_Naive_gate, Klrg1_gate)) %>%
...
xgb_pred_Klrg1 <- predict(xgb_Klrg1, test_matrix)
...
#view variable importance plot
test_importance_matrix <- xgb.importance(feature_names = colnames(train_matrix_step_Klrg1), model = xgb_Klrg1)
...
train_df <- train_df_step_Klrg1 %>% dplyr::select(-c(Time, File, File_scattered, cell_type, first_gate, second_gate, Res_Eff_Naive_gate)) %>% mutate(Klrg1_gate = Klrg1_gate %>% factor()) %>% as.data.frame()
test_df <- val_df_step_Klrg1 %>% dplyr::select(-c(Time, File, File_scattered, cell_type, first_gate, second_gate, Res_Eff_Naive_gate)) %>%  mutate(Klrg1_gate = Klrg1_gate %>% factor()) %>% as.data.frame()

# Do one hot encoding`<br/>
train_df <- createDummyFeatures(obj = train_df, target = "Klrg1_gate", method = "1-of-n")
test_df <- createDummyFeatures(obj = test_df, target = "Klrg1_gate", method = "1-of-n")

# Create tasks
train_task <- makeClassifTask(data = train_df, target = "Klrg1_gate")
test_task <- makeClassifTask(data = test_df, target = "Klrg1_gate")

# Create learner
lrn <- makeLearner("classif.xgboost", predict.type = "response")
lrn$par.vals <- list(objective = "multi:softprob", etest_metric = "merror", nrounds = 100L, booster = "gbtree", max_depth = 4, min_child_weight = 7.53, gamma = 4.07, subsample = 1, colsample_bytree = 1, nthread = 6)
