...
library(tictoc)
library(tidyverse)
path_to_files <- "~/PRIMUS/data/83_BIOINFORMATICS/Kadu/motif_ZNF644_Olha_Pyko/"
```
...
IDR_thresholded_peaks_list <- IDR_thresholded_peaks_files %>% purrr::map(read_delim,
  ...
  col_names = FALSE,
  trim_ws = TRUE
) %>% purrr::set_names("ENCFF176ATR", "ENCFF203RMY", "ENCFF309VMV", "ENCFF526JWC")
...
gene_bed_list <- IDR_thresholded_peaks_df %>% 
  ...
  arrange(desc(width)) %>% 
  dplyr::filter(width %in% c(331, 337)) %>% 
  split(f = .$width)
...
# Create fasta files
```{r}
sequence_list$`331` %>% Biostrings::writeXStringSet(filepath = paste0(path_to_files, "ZNF644_331_nt_length.fa"))
sequence_list$`337` %>% Biostrings::writeXStringSet(filepath = paste0(path_to_files, "ZNF644_337_nt_length.fa"))
```

# Run MEME-ChIP
```{r}
# time meme-chip -dna -minw 6 -maxw 15 ZNF644_331_nt_length.fa
# time meme-chip -dna -minw 6 -maxw 15 ZNF644_337_nt_length.fa
```
...
path_list_1 <- list.files(path = paste0(path_to_files, "ZNF644_331_nt_length_memechip_out"), pattern = "fimo", full.names = TRUE)
path_list_2 <- list.files(path = paste0(path_to_files, "ZNF644_337_nt_length_memechip_out"), pattern = "fimo", full.names = TRUE)

path_list <- c(path_list_1, path_list_2)
...
fimo_result_list[[4]] <- NULL
fimo_result_list[[32]] <- NULL
...
consensus_figure_list <- consensus_figure_list %>% purrr::set_names(c(paste0("331_nt_motif_", 1:19), paste0("337_nt_motif_", 1:46)))
...
ppm_list <- ppm_list %>% purrr::set_names(c(paste0("331_nt_motif_", 1:19), paste0("337_nt_motif_", 1:46)))
...
MOTIF ZNF644"
  ...
  writeLines(meme_out, paste0(path_to_files, "ZNF644_motif_", i, ".meme"))
})
```